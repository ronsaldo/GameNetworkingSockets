...
bool CConnectionTransportUDP::BConnectWithLocalPort( const netadr_t &netadrRemote, int localPort, SteamDatagramErrMsg &errMsg )
{
	// Create an actual OS socket which is bound to the specified local port.
	Assert( !m_pSocket );
	m_pSocket = OpenUDPSocketBoundToHostLocalPort( netadrRemote, localPort, CRecvPacketCallback( PacketReceived, this ), errMsg );
	if ( !m_pSocket )
		return false;
	return true;
}
...
bool CSteamNetworkConnectionUDP::BInitConnectWithLocalPort( const SteamNetworkingIPAddr &addressRemote, int localPort, int nOptions, const SteamNetworkingConfigValue_t *pOptions, SteamDatagramErrMsg &errMsg )
{
	AssertMsg( !m_pTransport, "Trying to connect when we already have a socket?" );
